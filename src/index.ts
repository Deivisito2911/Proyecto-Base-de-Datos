...
app.post("/boards/lists", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const text = "INSERT INTO list(name, boardId) VALUES($1, $2) RETURNING *";
...
app.post("/boards/lists/cards", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const cardText =
      "INSERT INTO card(title, description, due_date, list_id) VALUES($1, $2, $3, $4) RETURNING *";
    const date = new Date(cardDto.due_date);
    const cardValues = [
      cardDto.title,
      cardDto.description,
      date,//Pendiente con el ingreso de este formato de fecha , poner valores como 222 para probar el ingreso en la db
      cardDto.list_id,
    ];
    const cardResult = await client.query(cardText, cardValues);

    const cardUserText =
      "INSERT INTO card_user(isOwner, cardId, userId) VALUES($1, $2, $3)";
...
app.get("/boards/lists/cards", async (req: Request, res: Response) => {
  try {
    ...
    const text =
      'SELECT c.id, c.title, c.description, c.due_date, u.name "ownerUsername" FROM card c JOIN card_user cu ON c.id = cu.cardId JOIN users u ON cu.userId = u.id WHERE c.list_id = $1 AND cu.isOwner = true';
    const result = await pool.query(text, value);
    res.status(200).json(result.rows);
  } catch (errors) {
    res.status(422).json(errors);
